...
// London center coordinates for proper map centering
...
// Enhanced color scheme for better visibility matching London crime maps
...
// Map bounds component to ensure we're focused on London
const MapBounds = () => {
  const map = useMap();
  
  useEffect(() => {
    // Set initial view to London center
    map.setView(LONDON_CENTER, LONDON_ZOOM);
    
    // Set max bounds to keep focus on Greater London area
    const londonBounds = L.latLngBounds(
      L.latLng(51.28, -0.51), // Southwest corner
      L.latLng(51.69, 0.34)   // Northeast corner
    );
...
const EnhancedMapComponent: React.FC<EnhancedMapProps> = ({ 
  ...
  // Fetch boundary data from Flask backend based on current level
  useEffect(() => {
    const fetchBoundaryData = async () => {
      ...
      try {
        console.log(`Fetching ${currentLevel} level boundaries from Flask backend...`);
        
        const apiUrl = `http://localhost:5000/api/${currentLevel === 'borough' ? 'borough' : 'lsoa'}/boundaries`;
        ...
        if (geojsonData && geojsonData.type === 'FeatureCollection') {
          if (currentLevel === 'borough') {
            setBoroughData(geojsonData as BoroughGeoJSON);
          } else {
            setLsoaData(geojsonData as LSOAGeoJSON);
          }
          console.log(`Successfully loaded ${geojsonData.features?.length || 0} ${currentLevel} boundaries`);
        } else {
          throw new Error(`Invalid ${currentLevel} boundary data received`);
        }
      } catch (err: any) {
        console.error(`Error fetching ${currentLevel} boundaries:`, err);
        setError(err.message || `Failed to fetch ${currentLevel} boundaries from Flask backend`);
      }
  ...
  const onEachFeature = useCallback((feature: any, layer: L.Layer) => {
    ...
    // Create popup content based on current level
    ...
    <div className="relative h-full w-full">
      ...
      {loading && (
        <div className="absolute inset-0 z-[1000] bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-lg shadow-lg">
            ...
            <p className="mt-2 text-gray-700">Loading real {currentLevel} boundaries from Flask...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="absolute top-16 right-4 z-[1000] bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg max-w-sm">
          ...
          <p className="text-xs mt-2">Make sure Flask backend is running on localhost:5000</p>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 z-[1000] bg-white rounded-lg shadow-lg p-4">
        ...
        <div className="mt-2 pt-2 border-t border-gray-200">
          <p className="text-xs text-gray-500">
            Real London {currentLevel.toUpperCase()} boundaries with burglary data
          </p>
        </div>
      </div>

      {/* Map Container with real London geography */}
      <MapContainer
      ...
      >
        ...
        {currentData && (
          <GeoJSON
            ...
            key={`${currentLevel}-${currentData.features.length}`} // Force re-render on level change
          />
        )}
      </MapContainer>
    </div>
