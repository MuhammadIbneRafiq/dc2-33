...
def calculate_feature_relevance(model, dataloader, device, feature_names):
    model.eval()
    ig = IntegratedGradients(model)
    total_relevance = np.zeros(len(feature_names))
    total_samples = 0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            attributions, _ = ig.attribute(inputs, target=labels, return_convergence_delta=True)
            relevance_scores = torch.mean(torch.abs(attributions), dim=0).cpu().numpy()
            total_relevance += relevance_scores

    avg_relevance = total_relevance / total_samples
    relevance_dict = {feature_names[i]: float(avg_relevance[i]) for i in range(len(feature_names))}
    relevance_table = pd.DataFrame({"feature": list(relevance_dict.keys()), "relevance": list(relevance_dict.values())})

    # Save predictions to CSV
    relevance_table.to_csv(os.path.join(RESULTS_DIR, 'risk_predictions_LSTM.csv'), index=False)
    print(f"Prediction results saved at {RESULTS_DIR}")
...
def main():
    try:
        ...
        dataloader = DataLoader(Data, batch_size=32, shuffle=True) # Replace with your DataLoader logic
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        feature_names = feature_cols
        calculate_feature_relevance(model, dataloader, device, feature_names)
